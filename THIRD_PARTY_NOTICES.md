...
The explicitly inventoried Gpichan visuals and separately authorized project icons use CC BY 4.0, including permission for modifications and commercial reuse; see `distribution/ARTWORK-LICENSE.md` and `distribution/ARTWORK-SCOPE.json`. Icon artwork includes the listed tray PNG data embedded in TypeScript; code remains MIT. Other artwork and generated derivatives remain subject to their own source terms. Independent icon provenance remains in `distribution/ARTWORK-NOTICE.md`. Anime2.5DRig sample artwork and PSDs are deliberately excluded; its embedded generic parts remain under the upstream notice.
