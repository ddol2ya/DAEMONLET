...
The explicitly inventoried Gpichan visuals use CC BY 4.0, including permission for modifications and commercial reuse; see `distribution/ARTWORK-LICENSE.md` and `distribution/ARTWORK-SCOPE.json`. Other artwork and generated derivatives remain subject to their own source terms. The separately generated rabbit icons are not established Gpichan derivatives and receive no new grant here; their provenance remains in `distribution/ARTWORK-NOTICE.md`. Anime2.5DRig sample artwork and PSDs are deliberately excluded; its embedded generic parts remain under the upstream notice.
