...
Connection admission and session recovery are separate contracts. After admission, Backend emits a synchronous session-close event with the cause and whether a child existed. Service immediately detaches that backend with identity/epoch checks. A lost child requires explicit New conversation; the old transcript is never displayed as if a silently recreated fork had retained it. Drafts remain editable, and no failed or subsequent input is automatically resent. Initial pre-child auth failures may be retried by a new manual send after normal login. A malformed terminal response or known failed turn can leave the child alive; the next manual question then uses that same context. Reentrant disposal shares process cleanup so explicit reset cannot outrun termination.

The standalone `side-chat:check-parent` command reports source-home scope, safe file access, format, dynamic-tool presence and confirmed/absent/not-inspected terminal boundaries without starting a CLI or reading credentials. It is a diagnostic only, not an admission override. See the [recovery review record](side-chat-validation.md#review-of-8df6299-closed-child-recovery-2026-09-16).
