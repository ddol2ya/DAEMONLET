...
The verified **official CLI 0.154.0 / macOS arm64 / gpt-5.6-luna same-home profile** creates a separate ephemeral child in the parent's Codex Home. It explains successful completed context and explicitly selected project files and can propose changes for copying. File mutations, commands, builds/tests, external services and parent controls are blocked. File access uses a user selection, not automatic search. See [the new support record](docs/side-chat-official-readonly.md) for separately reported fixture, existing-parent and real-account evidence.

The setting remains OFF by default. Inherited context and selected files may be sent to the model provider and consume allowance. App chat/drafts stay in memory, while normal Codex storage, logging and authentication processing can occur. Hiding retains the conversation; reset, character and language changes create a new child. A saved custom executable needs explicit official selection; there is no silent fallback. Other OS/CLI versions, managed policy and keychain-only authentication require separate validation.
