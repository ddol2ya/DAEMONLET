...
**macOS arm64 · 공식 CLI 0.154.0의 검증된 실행파일 · gpt-5.6-luna 조합에서 실제 계정 응답을 확인했습니다.** 동적 도구가 없는 로컬 legacy 부모 대화만 지원하며, 새 paginated 저장 형식의 부모·다른 CLI 빌드·Windows는 아직 연결하지 않습니다. 설정은 기본 OFF이고, 첫 전송 전에 실행 제한·인증·모델 지원을 확인합니다. [지원·검증 기록](docs/side-chat-validation.md)을 확인하세요.

지원 런타임에서는 부모 대화 이력이 Codex 모델에 다시 전달되어 사용량이 발생할 수 있습니다. 대화 원문·초안은 메모리에만 보관하며, 숨기기는 유지, 초기화·캐릭터 적용·언어 전환은 새 대화, 기능 OFF·종료는 정리합니다. 파일/코딩 실행, 음성, 장기 기억, 네이티브 Codex 사이드 UI 동기화는 제공하지 않습니다. 페르소나 없는 기존 팩은 중립 프로필을 사용하고, 새 capability를 포함한 팩은 구형 앱에서 거부될 수 있습니다.
