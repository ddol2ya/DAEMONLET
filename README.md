...
**현재 검사한 Codex CLI 0.153.4는 실행 정책 검증에 실패해 실제 전송이 차단됩니다.** UI와 팩 페르소나 기능이 구현됐다는 사실은 실제 AI 연결 지원을 뜻하지 않습니다. 별도 API나 메인 작업 전송으로 대체하지 않습니다. [지원·검증 기록](docs/side-chat-validation.md)을 확인하세요.
