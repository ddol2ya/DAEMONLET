...
소스 빌드는 Git checkout에서 실행하며 **Node 24**를 검증 기준으로 사용합니다(선언된 최소 버전: 22.13). macOS에서는 음성 입력 helper 컴파일에 **Xcode Command Line Tools**가 필요합니다. `xcode-select -p`로 확인하고, 설치되지 않았다면 `xcode-select --install`을 실행하세요. Windows 소스 빌드에는 Visual Studio C++ 빌드 도구가 필요합니다. 일반 배포 앱이나 Hook 실행에는 Node·Xcode·C++ 빌드 도구가 필요하지 않습니다.
