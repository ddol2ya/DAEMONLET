...
The registered macOS arm64 / native CLI 0.154.0 / gpt-5.6-luna profile passed first-request policy, compaction and pre-execution tool controls plus a user-authorized real-account packaged test. Production creates temporary app-owned HOME, CODEX_HOME, working and temp directories with a restricted environment. It reads native effective config and managed requirements; any nonempty additional active config layer or managed requirements block connection. User Hook/MCP/plugin config is not imported. This does not bypass system/organization policy.

The auth broker reads only an existing private regular `auth.json` from the selected Codex home and passes an unexpired ChatGPT access token through official in-memory external-token login. It does not copy that file, use its refresh token, perform OAuth refresh, or write user credentials. An official refresh callback can only reuse a newly available unexpired token for the same account; otherwise normal Codex login is required. Authentication readiness alone is not proof of a model response.

The controlled parent reader validates the selected owned legacy rollout and reads its terminal boundary; the native ephemeral fork reads that selected path without copying its transcript into the new home. Paginated parents and inherited dynamic tools are rejected. The isolated process does not expose model file, image, shell, web, MCP, app, plugin or agent tools. stderr is drained, not logged. Child bodies remain memory-only in scoped CLI fixture disk checks; normal cleanup removes the app-owned temporary directories. A crash may leave configuration/cache directories; no crash-time full-disk absence claim is made.

Global Codex configuration, Hook/MCP configuration, credentials, signing state and running user sessions are not modified. Explicit account QA can save its non-sensitive test responses and screenshots to a chosen private evidence directory; ordinary startup cannot select that harness or a fake provider. See `docs/side-chat-validation.md` for actual scope and unresolved checks.
