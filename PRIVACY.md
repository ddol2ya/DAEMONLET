...
## Character conversations (PR #17 candidate)

New installations default to ON; existing explicit OFF choices remain OFF. Opening the feature or choosing Check again can start an app-owned restricted official Codex process for account, usage, model and configuration metadata. This preparation does not fork or generate a model answer. The first send requires versioned consent to inherited context, selected files, usage and storage. A send uses the selected parent's existing CODEX_HOME and a separate temporary working directory, forks through a successful completed turn, and communicates only with its owned temporary child. It does not resume, send to, interrupt or otherwise control the parent. Hiding, expanding and copying do not submit a model turn.

Inherited parent context and explicitly selected project excerpts may be sent to the configured official model provider and consume allowance. A fork inherits a snapshot, not live changes. Information already in the parent context is not removed by the new file-read restrictions. File-name/content checks do not detect all possible secrets in ordinary source files.

Main uses a restricted file picker service, never a model shell, to read selected text. It validates project scope, links, file identity, type and bounds. Excerpts carry line ranges and read times. Limits are 1 MiB per source file, 400 lines/32 KiB per excerpt, and eight files/128 KiB per send. Invalid reads return an error. Root, parent, account and policy changes invalidate prior attachment authority. The renderer receives relative names and excerpt metadata; the bytes remain in Main until sent to the same child.

Ordinary Daemonlet chat bodies, drafts and pending excerpts live in Main/renderer memory, not localStorage, IndexedDB, activity history or transcript logs. Explicit Copy writes the full chosen response to the clipboard. Limits remain 4,000 code points/16,000 UTF-8 bytes per input, 64 KiB per response text, 100 messages and 2 MiB of displayed history. Hiding retains the conversation. A lost child retains the cause and unsent draft and requires an explicit new conversation, without automatic retransmission.

The official engine may perform normal state/index/cache/lock/log/authentication processing in its original home. Ephemeral does not promise that nothing is ever retained by the engine, operating system or provider. This permission does not authorize model file writes, command execution, external services or parent control. Daemonlet neither edits Codex databases/settings nor deletes/restores original state to conceal changes. Source parent messages, successful boundaries, config/hooks and project files are separate preservation checks.

Native Codex authenticates using the existing supported ChatGPT credential path and may refresh credentials normally. Main uses native account/usage identity metadata; file storage also retains a protected account-ID check. Keyring/auto needs a backend-bound workspace ID and a fresh restricted metadata reader before sending, so external account switches are not hidden by a cached native process. Unavailable identity fails closed. Main never dumps protected stores, retains/injects tokens, copies auth files, issues login/logout, switches accounts or modifies global policy. CLI provenance, permissions and account/model availability are checked independently. Saved task-control selections remain untouched; a separate app preference can select the official side-chat CLI.
