...
The feature defaults to OFF. It starts an app-owned official Codex process only for an explicit send/preparation, using the selected parent's existing CODEX_HOME and a separate temporary working directory. It forks through a successful completed turn and communicates only with its owned ephemeral child. It does not resume, send to, interrupt or otherwise control the parent. Opening, hiding, expanding and copying do not submit a model turn.
...
Native Codex authenticates using the existing ChatGPT file credential path and may refresh credentials normally. Main reads only the account identifier for change detection and retains no token for injection. It does not copy auth files, issue login/logout, switch accounts, or modify global policy. Keychain-only and managed-policy configurations need separate validation. Runtime admission pins native bytes, platform, model availability and process/turn restrictions; it does not automatically replace a saved custom executable or fall back to another engine.

Persona data and file contents are untrusted descriptive material. They cannot grant permissions. The default app still contains only Gpichan; existing external packs, illustrations and rigs are preserved. A separately authorized QA harness can save its responses/screenshots to an explicit private review directory; normal app startup cannot select that harness or a fake provider. See [the official same-home record](docs/side-chat-official-readonly.md) for measured results and limits. Earlier isolated-home/custom records retain their historical scope.
