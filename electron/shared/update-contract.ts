...
export type UpdateAction = { action: "check" | "cancelDownload" | "openRelease" } | { action: "download" | "installAndRestart"; candidateId: string } | { action: "setUnsignedWindowsPolicy"; enabled: boolean }
export function parseUpdateAction(value: unknown): UpdateAction | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null
  const input = value as Record<string, unknown>
  if (input.action === "setUnsignedWindowsPolicy" && Object.keys(input).sort().join() === "action,enabled" && typeof input.enabled === "boolean") return input as UpdateAction
