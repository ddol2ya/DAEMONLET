...
export async function resolveOfficialParent(client: AppServerJsonlClient, parent: ChatParent) {
  const value: any = await client.request("thread/read", { threadId: parent.threadId, includeTurns: false }).catch(() => { throw Error("PARENT_ACCESS") })
  const thread = value?.thread
  if (thread?.id !== parent.threadId || thread.ephemeral !== false || typeof thread.cwd !== "string") throw Error("PARENT_UNSUPPORTED")
  const roots = await Promise.all([realpath(thread.cwd), realpath(parent.cwd)]).catch(() => { throw Error("PARENT_ACCESS") })
  if (roots[0] !== roots[1]) throw Error("PARENT_UNSUPPORTED")
  ...
  for (let n = 0; n < 10; n++) {
    ...
    if (!page.nextCursor) throw Error("PARENT_NO_COMPLETED_TURN")
  ...
  throw Error("PARENT_LOOKUP_LIMIT")
}
