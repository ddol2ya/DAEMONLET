...
export class CharacterIpcController {
  ...
  register() {
    ...
    for (const [channel, mode] of [[CHARACTER_IPC.remove, "remove"], [CHARACTER_IPC.rollback, "rollback"]] as const) this.bind(channel, 1, true, (owner, [v]) => this.nativeDialog(async () => {
      ...
      const result = await dialog.showMessageBox(o.settings.window!, { type: "question", title: mode === "remove" ? "캐릭터 제거" : "이전 버전 복원", message: entry.name,
        detail: mode === "remove" ? `${entry.version} 버전과 설치된 이전 버전을 제거합니다. 선택 중이면 기본 제공 캐릭터로 전환합니다.` : `${entry.version} → ${entry.previousVersion} 버전으로 복원합니다.`,
        buttons: ["취소", mode === "remove" ? "제거" : "복원"], defaultId: 0, cancelId: 0 })
