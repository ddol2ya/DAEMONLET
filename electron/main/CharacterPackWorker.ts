...
export type PackValidator = (task: PackValidationTask, signal?: AbortSignal, progress?: (value: PackProgress) => void) => Promise<ValidatedPack>
export function createPackValidator(workerPath: string): PackValidator {
  return (task, signal, progress) => new Promise((resolve, reject) => {
    ...
    worker.on("message", result => {
      if (done) return
      if (result.progress) { progress?.(result.progress); return }
      finish(result.ok ? undefined : new Error(result.code), result.value)
    })
