...
export class CharacterChatWindow {
 ...
 private initialize(){return this.initialized??=Promise.all([this.service.initialize(),this.layout.load()]).then(()=>{})}
 ...
 this.hooks.active(true);this.publish();win.once('ready-to-show',()=>{position();win.show();win.focus()});win.once('closed',()=>{if(this.layoutSaveTimer)void this.saveLayout();this.window=null;this.detach?.();this.detach=null;void this.service.close().catch(e=>this.service.setError(e));this.hooks.active(false);this.publish()});if(this.service.snapshot().character)await this.hooks.select(this.service.snapshot().character!);void win.loadURL(expectedRendererUrl('character-chat',this.devServerUrl))}
 async selectedCharacterChanged(id:string){const current=this.service.snapshot().character,entry=this.registry.get(id);if(this.window&&entry&&(current?.id!==id||current.revision!==entry.revision))await this.service.selectCharacter(id)}
 ...
 async dispose(){this.moveGesture.cancel();this.resizeGesture.cancel();ipcMain.removeHandler(LOCAL_CHAT_IPC.gesture);ipcMain.removeHandler(LOCAL_CHAT_IPC.action);ipcMain.removeHandler(LOCAL_CHAT_IPC.getPresentation);this.detach?.();await this.saveLayout();await this.service.close();this.window?.destroy();this.window=null}
}
