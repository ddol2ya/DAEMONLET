...
export class CharacterChatWindow {
 ...
 case 'layout-reset':this.moveGesture.cancel();this.resizeGesture.cancel();this.layout.reset();this.positionWindow?.();await this.saveLayout();break;case 'attention':if(typeof value.active!=='boolean')throw Error('잘못된 요청');this.service.attention(value.active);break;case 'memory-save':await this.service.saveMemory(value.text,value.id);break;case 'memory-delete':await this.service.deleteMemory(value.id);break;case 'snapshot':break;case 'send':await this.service.send(value.text);break;case 'stop':await this.service.stop();break;case 'retry':await this.service.retry();break;case 'new':await this.service.newChat();break;case 'conversation':await this.service.selectConversation(value.id);break;case 'delete':{
  ...
  if(!snapshot.conversations.some(c=>c.id===value.id&&c.characterId===snapshot.character?.id))throw Error('대화가 현재 캐릭터와 다릅니다.');
  this.deleting=true;try{const answer=await dialog.showMessageBox(win,{type:'question',message:'이 대화를 삭제할까요?',detail:'대화와 연결된 요약이 삭제됩니다. 별도로 저장한 기억은 유지됩니다.',buttons:['취소','삭제'],defaultId:0,cancelId:0,noLink:true});
   if(!this.disposed&&this.window===win&&!win.isDestroyed()&&answer.response===1)await this.service.deleteConversation(value.id);
  }finally{this.deleting=false}break;
 }case 'character':await this.service.selectCharacter(value.id);break;case 'model':await this.service.selectModel(value.id);break;
 case 'download':void this.service.installModel(value.id).catch(e=>this.service.setError(e));break;case 'cancel-download':await this.service.models.cancel();break;
 case 'import-model':{await this.service.requireRuntimeForInstall();const win=this.window;if(!win||win.isDestroyed())break;const result=await dialog.showOpenDialog(win,{title:'공식 GGUF 파일 가져오기',filters:[{name:'GGUF',extensions:['gguf']}],properties:['openFile']});if(this.window===win&&!win.isDestroyed()&&!result.canceled&&result.filePaths[0]){await this.service.installModel(value.id,result.filePaths[0])}break}
 case 'remove-model':await this.service.removeModel(value.id);break;
