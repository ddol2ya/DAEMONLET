...
import {mkdir,stat,statfs,copyFile,rm,open} from 'node:fs/promises'
...
export class ModelManager {
 ...
 async importFile(id:LocalModelId,source:string){if(this.operation)throw Error('모델 설치가 진행 중입니다.');const controller=this.controller=new AbortController();this.operation=(async()=>{await this.space(this.catalog[id].bytes);this.progress({model:id,bytes:0,total:this.catalog[id].bytes});const target=this.path(id)+'.import';try{if((await stat(source)).size!==this.catalog[id].bytes)throw Error('선택한 모델의 공식 GGUF 파일이 아닙니다.');controller.signal.throwIfAborted();await copyFile(source,target,constants.COPYFILE_FICLONE);controller.signal.throwIfAborted();if(await digestFile(target,controller.signal)!==this.catalog[id].sha256)throw Error('모델 SHA-256이 일치하지 않습니다.');controller.signal.throwIfAborted();await replaceFile(target,this.path(id),controller.signal);this.verified.delete(id)}finally{await rm(target,{force:true})}})();try{await this.operation}finally{this.operation=null;this.controller=null;this.progress(null)}}
 ...
 private async fetchModel(id:LocalModelId,signal:AbortSignal){const model=this.catalog[id],partial=this.path(id)+'.partial';let offset=await stat(partial).then(s=>s.size,()=>0);if(offset>model.bytes){await rm(partial);offset=0}await this.space(model.bytes-offset);this.progress({model:id,bytes:offset,total:model.bytes});
  ...
  signal.throwIfAborted();if(offset!==model.bytes||await digestFile(partial,signal)!==model.sha256){await rm(partial,{force:true});throw Error('모델 파일 검증에 실패했습니다. 다시 다운로드해 주세요.')}signal.throwIfAborted();await replaceFile(partial,this.path(id),signal);this.verified.delete(id)
 }
