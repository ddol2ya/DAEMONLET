...
& $CMake -S $LlamaSource -B "$BuildRoot/cuda" -G 'Visual Studio 17 2022' -A x64 -T 'cuda=13.0' -DGGML_CUDA=ON -DGGML_NATIVE=OFF '-DCMAKE_CUDA_ARCHITECTURES=75;86;89' -DBUILD_SHARED_LIBS=OFF -DGGML_OPENMP=OFF -DLLAMA_OPENSSL=OFF -DLLAMA_BUILD_TESTS=OFF -DLLAMA_BUILD_EXAMPLES=OFF -DLLAMA_BUILD_SERVER=ON -DLLAMA_BUILD_TOOLS=ON '-DCMAKE_MSVC_RUNTIME_LIBRARY=MultiThreaded'
