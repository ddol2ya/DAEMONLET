...
import {snapshotRun} from './production-review.mjs'
...
const snapshot = await snapshotRun(source)
if(values.pose&&!snapshot.poses[values.pose])throw Error('Unknown selected pose: '+values.pose)
await mkdir(dirname(output),{recursive:true});await mkdir(output)
await writeFile(resolve(output,'source-snapshot.json'),JSON.stringify(snapshot,null,2)+'\n')
...
try {
 ...
 if(snapshot.fingerprint!==(await snapshotRun(source)).fingerprint){
  await writeFile(resolve(output,'failure.json'),JSON.stringify({error:'Source changed during capture; recapture selected assets'}))
  process.exitCode=1
 }
