...
app.whenReady().then(async () => {
  ...
  try {
    ...
    idleId = await run(`const u=new URL('/characters/'+${q(process.env.INDEPENDENT_QA_ID)}+'/character.json',location.href),c=await(await fetch(u)).json(),b=c.behavior?await(await fetch(new URL(c.behavior,u))).json():{};return b.states?.NORMAL?.poseId??'base'`)
    ...
    if(initial.poses.length!==10)throw new Error('Missing poses')
    ...
    await at('mouseDown','face');await delay(65);await at('mouseUp','face');await active('head-tap');await capture('13-head-tap','head-tap');await active(idleId)
    await at('mouseDown','torso');await delay(65);await at('mouseUp','torso');await active('torso-tap');await capture('14-torso-tap','torso-tap');await active(idleId)
    await at('mouseDown','petStart');await at('mouseMove','petEnd');await active('head-pet');await capture('15-head-pet','head-pet')
    ...
    await at('mouseDown','petStart','writing');await at('mouseMove','petEnd','writing');await delay(450);await capture('18-busy-pet','writing')
