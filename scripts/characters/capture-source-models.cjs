...
app.whenReady().then(async()=>{
 ...
 const run=code=>win.webContents.executeJavaScript(`(async()=>{${code}})()`,true),reports=[]
 try{
  ...
  for(const model of models){
   ...
   for(const test of process.env.SOURCE_QA_CONTACTS==='1'?[]:cases){
    ...
    fs.writeFileSync(path.join(dir,test.id+'.png'),Buffer.from(png.split(',')[1],'base64'));records.push(test)
   }
   let motionFrames=0
   if(process.env.SOURCE_QA_CONTACTS==='1'){
    ...
    const metrics=await run(`const z=qa,auth=${q(authored)},duration=z.pose.motion.playback==='once'?z.pose.motion.loopDurationMs+1000:z.pose.motion.loopDurationMs,frames=[];const sample=(name,p)=>{const l=z.renderer.poseLayers.find(l=>l.name===name);if(!l)throw Error('Missing contact layer '+name);const u=(p[0]-l.x)/l.w*l.nx,v=(p[1]-l.y)/l.h*l.ny,ix=Math.max(0,Math.min(l.nx-1,Math.floor(u))),iy=Math.max(0,Math.min(l.ny-1,Math.floor(v))),tx=u-ix,ty=v-iy,at=(a,b,k)=>l.current[((iy+b)*(l.nx+1)+ix+a)*2+k];return [0,1].map(k=>(at(0,0,k)*(1-tx)+at(1,0,k)*tx)*(1-ty)+(at(0,1,k)*(1-tx)+at(1,1,k)*tx)*ty)};for(let frame=0;frame<=Math.ceil(duration*30/1000);frame++){const t=Math.min(duration,frame*1000/30),s=z.samplePoseMotion(z.pose.motion,t);z.renderer.setPoseLayerTransforms(s.layers);z.renderer.render({...z.DEFAULT_PARAMETERS,...s.parameters},200000+t,false);const points=auth.landmarks.map(m=>({name:m.name,tip:sample(m.layer,m.tip),root:sample(m.layer,m.root),bodyRoot:sample('topwear',m.root),head:m.targetLayer?sample(m.targetLayer,m.targetPoint??m.tip):m.name==='hand-hair-contact'?sample('front hair',m.tip):null}));if(points.some(p=>[...p.tip,...p.root,...p.bodyRoot,...(p.head??[])].some(v=>!Number.isFinite(v))))throw Error('Invalid contact vertex');frames.push({atMs:t,points})}return {frames,onceHolds:z.pose.motion.playback!=='once'||JSON.stringify(z.samplePoseMotion(z.pose.motion,z.pose.motion.loopDurationMs))===JSON.stringify(z.samplePoseMotion(z.pose.motion,z.pose.motion.loopDurationMs+2000))};`)
    const distance=(a,b)=>Math.hypot(a[0]-b[0],a[1]-b[1]);const contact=metrics.frames.flatMap(f=>f.points.filter(p=>p.head).map(p=>distance(p.tip,p.head)))
    const maxContact=contact.length?Math.max(...contact):null
    const traces=authored.landmarks.map((m,i)=>{const travel=Math.max(...metrics.frames.map(f=>distance(f.points[i].tip,metrics.frames[0].points[i].tip)));return {name:m.name,maxTipTravelPx:travel,travelAt320px:travel/4,travelAt460px:travel*460/1280,maxRootToBodyPx:Math.max(...metrics.frames.map(f=>distance(f.points[i].root,f.points[i].bodyRoot)))}})
    const summary={pose:model.id,frames:metrics.frames.length,onceHolds:metrics.onceHolds,maxHandHeadFieldDifferencePx:maxContact,traces}
    ...
    if(!metrics.onceHolds||maxContact!==null&&maxContact>2)throw Error('Contact or once-hold audit requires correction')
   }
   ...
   const report={pose:model.id,diagnostic,cases:records,motionFrames,resourcesAfterUnload:resources};fs.writeFileSync(path.join(dir,'capture.json'),JSON.stringify(report,null,2));reports.push(report);console.log(JSON.stringify({pose:model.id,staticFrames:records.length,motionFrames,vertices:diagnostic.vertices}))
  }
  fs.writeFileSync(path.join(output,'capture.json'),JSON.stringify({status:'captured_pending_visual_review',poses:reports.map(r=>({pose:r.pose,staticFrames:r.cases.length,motionFrames:r.motionFrames}))},null,2))
