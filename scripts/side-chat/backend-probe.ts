...
const { values } = parseArgs({ options: {
  codex: { type: "string" }, output: { type: "string" }, history: { type: "string", default: "paginated" },
  parentCompaction: { type: "boolean", default: false }, toolHistory: { type: "boolean", default: false },
  nativeLineage: { type: "boolean", default: false }, concurrentParent: { type: "boolean", default: false },
  hostileParent: { type: "boolean", default: false }, compaction: { type: "boolean", default: false },
} })
if (!values.codex || !values.output || !["legacy", "paginated"].includes(values.history)) throw Error("--codex, --output and supported --history required")
...
const connect = async () => {
  const startup = replyIndex >= 0 ? await inspectOfficialStartup(state) : null
  const connection = replyIndex >= 0
    ? await launchOfficialFixture({ executable: values.codex!, root: join(root, "official-process"), codexHome: state, osHome: join(root, "home"), disabledMcpServers: startup!.disabledMcpServers })
    : await launchFixtureParent({ executable: values.codex!, root, execution: { model: "gpt-5.6-luna", instructions: "collaboration-mode", noEnvironment: true } }); connections.push(connection)
  ...
  if (startup) {
    ...
    const verify = async () => {
      ...
      try { assertOfficialConfiguration(requirements, { ...(effective as any), config: { ...(effective as any).config, model_provider: "openai" } }) }
...
try {
  ...
  await writeFile(join(state, "config.toml"), stringify({ ...FIXTURE_CONSTRAINTS, model: "gpt-5.6-luna", ...(values.compaction ? { model_auto_compact_token_limit: 100000 } : {}), model_provider: "fixture", model_providers: { fixture: fixtureProvider } }), { mode: 0o600 })
  ...
  let { thread: parent } = await seed.client.request("thread/start", { cwd, historyMode: values.history, ...(values.hostileParent ? { dynamicTools: [{ name: "forbidden_parent_tool", description: "Synthetic inherited capability", inputSchema: { type: "object", properties: {} } }] } : {}), sandbox: "read-only", approvalPolicy: "never", developerInstructions: "PARENT_POLICY_OLD_72bde" }) as any
  ...
  service.setCandidates([{ threadId: parent.id, title: "Synthetic parent", cwd }], parent.id)
  report.exchanges = []
  for (let n = 0; n < replies.length; n++) {
    ...
    if (n === 1) report.checks.userSelectedFileDelivered = users.includes("SELECTED_PROJECT_LINE_42") && users.includes('1: export const marker') ? "PASS" : "FAIL"
  ...
  if (true) {
    ...
    report.checks.activeParentExcluded = !firstInput.includes("ACTIVE_PARENT_EXCLUDED_5bf3") ? "PASS" : "FAIL"
  ...
  report.checks.nativeExecutorsAbsent = report.exchanges.length === replies.length && report.exchanges.every((e: any) => e.toolNames.every((name: string) => values.hostileParent && name === "forbidden_parent_tool")) ? "PASS" : "FAIL"
