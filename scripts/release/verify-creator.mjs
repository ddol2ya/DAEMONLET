...
// Update-source migration and feed generation must work from the extracted ZIP alone.
await readFile(join(skill, 'references/updates.md'))
await exec(process.execPath, [join(skill, 'scripts/creator.mjs'), 'add-update-source', '--input', join(target, 'persona-updated.petchar'), '--version', '0.7.2', '--repo-id', 'fixture/character-packs', '--manifest-path', 'updates/fixture/stable.json', '--output', join(target, 'update-source.petchar'), '--report', join(target, 'update-source.json')], {cwd: target, env, timeout: 240000, maxBuffer: 4 * 1024 ** 2})
const updateSource = JSON.parse(await readFile(join(target, 'update-source.json'), 'utf8'))
if (JSON.stringify(updateSource.changedFiles) !== JSON.stringify(['pack.json']) || updateSource.personaChanges !== 0 || updateSource.validation.outputImport !== 'PASS') throw Error('Standalone update source migration failed')
await writeFile(join(target, 'changes.txt'), 'Private standalone fixture; never publish.\n')
await exec(process.execPath, [join(skill, 'scripts/creator.mjs'), 'create-update-feed', '--pack', join(target, 'update-source.petchar'), '--artifact-path', 'packs/fixture/0.7.2.petchar', '--artifact-revision', 'a'.repeat(40), '--min-app-version', '0.7.2', '--notes-file', join(target, 'changes.txt'), '--output', join(target, 'stable.json')], {cwd: target, env, timeout: 240000, maxBuffer: 4 * 1024 ** 2})
const updateFeed = JSON.parse(await readFile(join(target, 'stable.json'), 'utf8'))
if (updateFeed.artifact.sha256 !== digest(await readFile(join(target, 'update-source.petchar'))) || updateFeed.version !== '0.7.2') throw Error('Standalone feed generation failed')
...
const result = {status: 'creator-extracted-verified', archive, sha256: digest(await readFile(archive)), extractedTo: target, files: count, bytes, runtimeCheck: check.technicalReadiness, personaValidation: 'PASS', personaMigration: 'PASS', updateSourceMigration: 'PASS', updateFeedGeneration: 'PASS', licenseReview: check.licenseReview.status, export: join(target, 'gpichan.petchar'), exportSha256: digest(await readFile(join(target, 'gpichan.petchar'))), fixturePreparation: 'Only unselected dialogue pose references removed from temporary copy; original artwork and notice preserved', pythonHelpers: 'loaded', renderer: buildOutput.trim(), inferenceRun: false}
