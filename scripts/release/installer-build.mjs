...
for (const file of artifacts.filter(file => file.endsWith('.exe'))) {
  ...
  if (publisherName) {
    ...
    if (await verifySignature([publisherName], file, console)) throw Error('Final installer signature verification failed')
  }
...
await writeFile(join(root, 'installer-build-result.json'), JSON.stringify({ schemaVersion: 1, kind: 'windows-installer-build', source: manifest.source, appVersion: manifest.appVersion, files, runtimeUnchanged: true, creatorToolsIncluded: false, signing: publisherName ? 'verified' : 'unsigned', publisherName }, null, 2) + '\n')
