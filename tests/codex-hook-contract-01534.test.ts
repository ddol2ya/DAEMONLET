...
const fixtures = JSON.parse(readFileSync("tests/fixtures/codex-hooks/contract-v0.153.4.json", "utf8")) as {
  codexVersion: string; artifactSha256: string
  events: { event: string; schema: Record<string, unknown>; input: Record<string, unknown> }[]
}

describe("Codex 0.153.4 Hook wire compatibility", () => {
  it("binds the reviewed schema fixtures to one exact native artifact and version", () => {
    ...
    expect(contract?.id).toBe("codex-cli-0.153.4-darwin-arm64")
