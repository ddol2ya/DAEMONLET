...
describe("explicit project excerpts without a shell", () => {
  ...
  it("preserves unsent draft/excerpts through owned character failure, apply and rollback, but clears them on parent change", async () => {
    const f = await fixture(), factory = vi.fn(() => { throw Error("No model request expected") })
    const service = new SideChatService(factory)
    const binding = (revision: string) => ({ id: "style-a", revision, label: "Style A", compiled: compilePersona("Style A", neutralPersona(), "ko") })
    service.configure(true, "ko"); service.setConnectionMode("official-same-home"); service.applyPersona(binding("before"))
    service.setCandidates([{ threadId: "parent", title: "fixture", cwd: f.project, sourceHome: f.state }], "parent")
    service.setDraft("unsent draft")
    await service.attachFile(join(f.project, "code.ts"), 1, 2, service.snapshot().epoch)
    const attached = service.snapshot().attachments
    const failed = service.beginCharacterApply(); service.characterFailed(failed)
    expect(service.snapshot()).toMatchObject({ draft: "unsent draft", attachments: attached })
    const beforeEpoch = service.snapshot().epoch, applied = service.beginCharacterApply()
    service.applyPersona(binding("after"), applied)
    expect(service.snapshot()).toMatchObject({ draft: "unsent draft", attachments: attached, messages: [] })
    expect(service.snapshot().epoch).toBeGreaterThan(beforeEpoch)
    const restored = service.beginCharacterApply(); service.applyPersona(binding("before"), restored)
    expect(service.snapshot()).toMatchObject({ draft: "unsent draft", attachments: attached, messages: [] })
    service.clearParent(); expect(service.snapshot().attachments).toEqual([])
