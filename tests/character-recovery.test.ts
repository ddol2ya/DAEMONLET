...
function exitFixture(c: any, disposePack: () => Promise<void> = async () => {}) {
  ...
  for (const name of ["codexUsage", "codexUsageIpc", "characterChat", "sideChatIpc", "activityIpc", "bubbleIpc", "taskControlIpc", "settingsIpc", "updateIpc", "packUpdateIpc", "characterIpc", "activityTitles", "protocol"]) c[name] = { dispose: vi.fn() }
