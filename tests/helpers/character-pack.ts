...
export function packFiles(options: { id?: string; version?: string; color?: number; extra?: Record<string, unknown> } = {}): ZipEntry[] {
  ...
  const manifest = { packFormatVersion: 1, id: character.id, name: character.label, entry: "character.json", version: options.version ?? "1.0.0", runtime: PACK_RUNTIME, files: entries.map(e => ({ path: e.path, bytes: e.data.length, sha256: sha256(e.data) })), ...options.extra }
