...
describe('updater metadata from packaged provenance', () => {
  it('uses the real ZIP source schema and emits review-only metadata', async () => {
    ...
    const result = await createUpdateMetadata({artifact: join(f.root, 'outputs', file), output: join(f.root, 'outputs/feed'), platform: 'darwin', minimumSystemVersion: '22.0.0', review: true})
  ...
  it('rejects a packaged dirty source even in review mode', async () => {
    ...
    await zip(f.root, file, [['Daemonlet for Codex.app/Contents/Resources/app.asar', await readFile(path)]])
    await expect(createUpdateMetadata({artifact: join(f.root, 'outputs', file), output: join(f.root, 'outputs/feed'), platform: 'darwin', minimumSystemVersion: '22.0.0', review: true})).rejects.toThrow('Clean stable source identity')
  })
  it('binds Windows metadata to the EXE hash and explicit unsigned policy', async () => {
    ...
    const input = {artifact: join(output, name), output: join(output, 'feed'), platform: 'win32', minimumSystemVersion: '10.0.19045', packagingResult: receipt}
