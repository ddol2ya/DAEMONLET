...
it('explicit model install cancellation is quiet and a new installation clears the old error',async()=>{
 const {service}=await fixture()
 service.setError(Error('previous transfer failed'))
 vi.spyOn(service.models,'download').mockRejectedValueOnce(new DOMException('This operation was aborted','AbortError'))
 await expect(service.installModel('E4B')).resolves.toBeUndefined()
 expect(service.snapshot().error).toBeNull()
 service.setError(Error('another old failure'))
 vi.mocked(service.models.download).mockImplementationOnce(async()=>{expect(service.snapshot().error).toBeNull()})
 await service.installModel('E4B')
 expect(service.snapshot().error).toBeNull()
})
it('closing during model inventory does not start a late runtime availability probe',async()=>{
 const {service}=await fixture(undefined,{initialize:false}),inventory=deferred<Array<'E4B'|'12B'>>()
 vi.mocked(service.models.installed).mockImplementationOnce(()=>inventory.promise)
 const initial=service.initialize()
 await vi.waitFor(()=>expect(service.models.installed).toHaveBeenCalled())
 const closed=service.close();inventory.resolve(['E4B'])
