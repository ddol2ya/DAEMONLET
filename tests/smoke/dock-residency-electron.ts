...
async function main() {
  ...
  const controller = new DockResidencyController({ app, dockVisible: () => app.dock!.isVisible(), windows: () => BrowserWindow.getAllWindows(), utilityWindows: () => [settings, lab],
  ...
  try {
    ...
    app.setActivationPolicy("regular"); await until(() => app.dock!.isVisible())
    await until(() => !app.dock!.isVisible())
    await writeFile(process.env.DAEMONLET_DOCK_RESULT!, JSON.stringify({ status: "PASS", electron: process.versions.electron,
      checks: ["native Dock visible during recovery window", "last utility close hides Dock with pet alive", "new utility window lifecycle", "external activation-policy drift corrected", "tray recovery hides Dock without closing UI"],
