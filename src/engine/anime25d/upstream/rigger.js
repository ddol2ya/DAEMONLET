...
(function (root, factory) {
  ...
  // Local adapter extension: preserve small semantic features while retaining
  // the upstream 40px default for unspecified/full-canvas layers.
  function cleanupThreshold(name, opts) {
    var values = opts && opts.cleanupThresholds;
    if (!values) return 40;
    var normalized = normName(name), base = baseName(normalized);
    var value = values[normalized];
    if (value == null) value = values[base];
    value = Number(value);
    return Number.isFinite(value) ? Math.max(0, Math.round(value)) : 40;
  }
  ...
  function dilate(mask, W, H, r) {
    // Binary box dilation with rolling sums. Preserve the exact clipped square
    // neighborhood while visiting each pixel a constant number of times.
    var tmp = new Uint8Array(W * H), out = new Uint8Array(W * H);
    var columns = new Uint32Array(W), x, y, k, count, o;
    for (y = 0; y < H; y++) {
      o = y * W; count = 0;
      for (k = 0; k <= r && k < W; k++) count += mask[o + k] ? 1 : 0;
      for (x = 0; x < W; x++) {
        tmp[o + x] = count ? 1 : 0;
        if (x - r >= 0) count -= mask[o + x - r] ? 1 : 0;
        if (x + r + 1 < W) count += mask[o + x + r + 1] ? 1 : 0;
      }
    }
    for (k = 0; k <= r && k < H; k++) {
      o = k * W;
      for (x = 0; x < W; x++) columns[x] += tmp[o + x];
    }
    for (y = 0; y < H; y++) {
      o = y * W;
      for (x = 0; x < W; x++) out[o + x] = columns[x] ? 1 : 0;
      if (y - r >= 0) {
        o = (y - r) * W;
        for (x = 0; x < W; x++) columns[x] -= tmp[o + x];
      }
  ...
  function buildRig(psd, opts) {
    ...
    for (var i = 0; i < kids.length; i++) {
      ...
      var fa = cleanAlpha(fullAlphaOf(kids[i], W, H), W, H, cleanupThreshold(kids[i].name, opts));
    ...
    if (faceE) {
      ...
      if (fb && fc) FACE = { cx: fc.cx, cy: fc.cy, x0: fb.x0, x1: fb.x1, y0: fb.y0, y1: fb.y1 };
      else warnings.push('face レイヤーが cleanup 後に空です — キャンバス中央を顔とみなします');
    }
    if (!FACE) {
      if (!faceE) warnings.push('face レイヤーがありません — キャンバス中央を顔とみなします');
    ...
    ['l', 'r'].forEach(function (s) {
      var ew = sided['eyewhite|' + s], ir = sided['irides|' + s], lash = sided['eyelash|' + s], ec = sided['eye_close|' + s];
      var eyeShape = ew || lash || ir;
      var K = 'eye' + s.toUpperCase();
      if (eyeShape) {
        var b = bboxOf(eyeShape, W, H, 8);
        if (!b) return;
        var a = { x0: b.x0, x1: b.x1, y0: b.y0, y1: b.y1 };
        var ic = ir ? centroidOf(ir, W, H) : centroidOf(eyeShape, W, H);
        if (!ic) return;
        a.icx = ic.cx; a.icy = ic.cy;
        var cc = ec ? centroidOf(ec, W, H) : null;
        a.closeY = cc ? cc.cy : (b.y0 + (b.y1 - b.y0) * 0.62);
        anchors[K] = a;
        if (!ew) warnings.push(K + ' anchor inferred from eyelash/irides because eyewhite is missing');
      }
    });
    ...
    if (mSrc) {
      ...
      if (mb && mc) anchors.mouth = { x0: mb.x0, x1: mb.x1, y0: mb.y0, y1: mb.y1, cx: mc.cx, cy: mc.cy };
    }
    if (!anchors.mouth) {
  ...
  function cleanPsdLayers(psd, opts) {
    ...
    for (var i = 0; i < kids.length; i++) {
      ...
      cleanAlpha(a, W, H, cleanupThreshold(c.name, opts));
