...
const formatTime = (at: number, locale: string) => new Intl.DateTimeFormat(locale, { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }).format(at)
...
export default function ActivityApp() {
  ...
  const groups = snapshot ? [
    { title: t("확인이 필요한 작업"), id: "attention", items: snapshot.entries.filter(r => r.state === "waiting" || r.unread) },
    { title: t("진행 중"), id: "running", items: snapshot.entries.filter(r => r.state === "running") },
    { title: t("최근 이력"), id: "history", items: snapshot.entries.filter(r => r.state !== "waiting" && r.state !== "running" && !r.unread) },
  ] : []
  ...
  >
    <header className="activity-header">
      <div className="heading-row"><h1 id="activity-title" ref={titleRef} tabIndex={-1}>{t("작업 목록")}</h1>
        <span className={`connection ${snapshot?.connection === "READY" ? "connected" : ""}`}>{snapshot ? t(connectionText[snapshot.connection]) : t("불러오는 중")}</span>
      </div>
    </header>
    {error && <p role="alert" className="banner error">{t(error)}</p>}
    {snapshot ? <>
      <div className="counts" aria-label={t("작업 상태 요약")}>
        {([ ["waiting", t("입력 필요")], ["failed", t("미확인 실패")], ["completed", t("미확인 종료")], ["running", t("실행 중")] ] as const).map(([key, label]) => <div className={`count ${key}`} key={key}><span>{label}</span><strong>{snapshot.counts[key]}</strong></div>)}
      </div>
      <p className="sr-only" role="status" aria-live="polite" aria-atomic="true">{activitySummary(snapshot, t.language)}</p>
      {snapshot.connection !== "READY" && <p className="banner">{t("연결 확인 중 · 마지막 수신 ")}{snapshot.lastObservedAt === null ? t("없음") : formatTime(snapshot.lastObservedAt, t.locale)}</p>}
      {snapshot.storage === "error" && <p className="banner error" role="alert">{t("이력 저장 실패 · 현재 목록은 유지되지만 앱을 다시 시작하면 최근 변경이 사라질 수 있습니다.")}</p>}
      {snapshot.historyRecovered && <p className="banner">{t("손상된 이력 파일을 격리했습니다. 이전 기록 일부를 복원하지 못했습니다.")}</p>}
      {snapshot.droppedUnread > 0 && <p className="banner">{t`보관 한도로 미확인 결과 ${snapshot.droppedUnread}건이 정리되었습니다.`}{snapshot.lastPrunedAt !== null && t` 최근 정리: ${formatTime(snapshot.lastPrunedAt, t.locale)}.`}</p>}
      {snapshot.capacityLimited && <p className="banner">{t("수집 또는 중복 방지 용량 한도에 도달한 적이 있습니다. 일부 기록이 누락되거나 다시 표시될 수 있습니다.")}</p>}
      <div className="toolbar">
        <button className="primary" disabled={pending || targets.length === 0} onClick={() => void acknowledge(targets)}>{t("미확인 결과 모두 확인")}{targets.length ? ` (${targets.length})` : ""}</button>
        <button disabled={pending || snapshot.navigation === "none"} onClick={() => void openCodex()}>{t("ChatGPT 앱 열기 ")}<span aria-hidden="true">↗</span></button>
      </div>
      {snapshot.navigation === "none" && <p className="navigation-note">{t("ChatGPT 앱을 찾지 못했습니다.")}</p>}
      {!snapshot.entries.length && <section className="empty"><span className="empty-symbol" aria-hidden="true">☷</span><h2>{t("아직 작업이 없습니다")}</h2></section>}
      ...
      <footer>
        <p>{snapshot.storage === "pending" ? t("이력 저장 중…") : snapshot.storage === "saved" ? t("로컬 이력 저장됨") : t("이력 저장 실패")}</p>
        <details><summary>{t("기록 보관 안내")}</summary><p>{t`종료 이력은 최대 ${ACTIVITY_HISTORY_LIMIT}건, ${ACTIVITY_HISTORY_DAYS}일 보관합니다. 미확인 결과도 보관 한도에 따라 정리됩니다. 대화 제목과 본문은 저장하지 않습니다.`}</p></details>
      </footer>
    </> : !error && <p className="empty" role="status">{t("작업 목록을 불러오고 있습니다…")}</p>}
    <p className="sr-only" role="status" aria-live="polite">{t(notice)}</p>
  </main>
}
...
function ActivityRow({ entry: r, pending, onConfirm, onOpen, onOpenAndConfirm }: { entry: ActivityEntry; pending: boolean; onConfirm: () => void; onOpen: () => void; onOpenAndConfirm: () => void }) {
  const t = useT()
  return <article className={`activity-row ${r.state}`} aria-label={`${r.name}, ${t(stateText[r.state])}`}>
    <span className="state-dot" aria-hidden="true" />
    <div className="row-content">
      <div className="row-title"><h3>{r.name}</h3><span className="state-label">{t(stateText[r.state])}{r.freshness === "rechecking" ? t(" · 재확인 중") : ""}</span>{r.unread && <span className="unread">{t("미확인")}</span>}</div>
      <p className="row-time">{t("첫 수신 ")}<time dateTime={new Date(r.firstObservedAt).toISOString()}>{formatTime(r.firstObservedAt, t.locale)}</time> · {r.endedAt === null ? t("최근 수신") : t("종료 시각")} {formatTime(r.endedAt ?? r.lastObservedAt, t.locale)}</p>
      {r.state === "unknown" && <p className="row-note">{t("종료 상태를 확인하지 못했습니다.")}</p>}
      {r.state === "waiting" && <p className="row-note">{r.freshness === "rechecking" ? t("입력 대기 상태를 다시 확인하고 있습니다.") : t("Codex에서 응답해 주세요.")}</p>}
      {r.state === "running" && r.category && <p className="row-note">{t(categoryText[r.category])}</p>}
      {r.acknowledgedAt !== null && <p className="row-note">{t`확인: ${formatTime(r.acknowledgedAt, t.locale)}`}</p>}
    </div>
    <div className="row-actions">
      {r.canOpenConversation && <button disabled={pending} aria-label={t`${r.name} 대화 열기`} onClick={onOpen}>{t("대화 열기 ↗")}</button>}
      {r.unread && <button className="confirm" title={t("이 결과만 확인")} disabled={pending} aria-label={t`${r.name} 결과 확인`} onClick={onConfirm}>{t("확인")}</button>}
      {r.unread && r.canOpenConversation && <button disabled={pending} onClick={onOpenAndConfirm}>{t("열고 확인")}</button>}
    </div>
  </article>
}
