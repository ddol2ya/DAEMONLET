...
export function SideChatApp({ api, onSnapshot }: { api: SideChatApi; onSnapshot?: (snapshot: SideChatSnapshot) => void }) {
  ...
  return <section className={panel ? "chat panel" : "chat compact"} hidden={snapshot.mode === "hidden"} aria-label={t.chat} onKeyDown={event => { if (event.key === "Escape" && !composing.current && !isComposing(event.nativeEvent)) { event.preventDefault(); event.stopPropagation(); void action("draft", draft).then(() => action("hide")) } }}>
    <div className="setup-region">
      <details className="conversation-options" open={!snapshot.parent || !snapshot.enabled || snapshot.readiness?.phase === "blocked"}>
        <summary>{ko ? "대화 설정" : "Conversation settings"}</summary>
        <section className="context"><label>{ko ? "작업" : "Task"}<select aria-label={t.parent} value={snapshot.parent?.handle ?? ""} disabled={busy} onChange={event => void action("parent", event.target.value)}><option value="" disabled>{t.empty}</option>{snapshot.parent && !snapshot.candidates.some(c => c.handle === snapshot.parent?.handle) && <option value={snapshot.parent.handle}>{snapshot.parent.title}</option>}{snapshot.candidates.map(parent => <option key={parent.handle} value={parent.handle}>{parent.title}</option>)}</select></label></section>
        <details className="parent-search"><summary>{ko ? "다른 작업 찾기" : "Find another task"}</summary>
          ...
          <button disabled={busy} onClick={() => { setQuery(""); void action("search", "") }}>{ko ? "새로고침" : "Refresh"}</button>
    ...
    <div className="history" ref={history} onScroll={() => { if (!panel) return; const e = history.current!; atBottom.current = e.scrollHeight - e.scrollTop - e.clientHeight < 36 }} aria-label={t.chat}>
      {panel ? snapshot.messages.map(message => <article key={message.id} className={"message " + message.role}><Answer text={message.text} /><button className="copy" onClick={() => void action("copy", message.id)}>{t.copy}</button></article>) : latest ? <article className="message assistant"><div className="answer-text">{long ? latest.preview && previewFits ? latest.preview : t.full : latest.text}</div>{long && <button className="expand" onClick={() => void action("panel")}>{t.expand} ↗</button>}{!long && <button className="copy" onClick={() => void action("copy", latest.id)}>{t.copy}</button>}</article> : null}
    </div>
    <div role="status" className="response-status">{snapshot.applying ? t.applying : snapshot.phase === "answering" ? t.answering : snapshot.phase === "preparing" ? t.preparing : snapshot.readiness?.phase === "checking" ? (ko ? "연결 확인 중…" : "Checking connection…") : ""}</div>
    {issue && <div className="error" role="alert">{errors[issue]?.[snapshot.language === "ko" ? 0 : 1] ?? issue}{snapshot.requiresNewConversation && <button className="recovery" onClick={() => void action("reset")}>{t.reset}</button>}</div>}
    <footer><label className="sr-label" htmlFor="chat-input">{t.input}</label><textarea id="chat-input" placeholder={ko ? snapshot.character.label + "에게 물어보기" : "Ask " + snapshot.character.label} disabled={!snapshot.enabled} rows={2} value={draft} onChange={e => { if (validChatInput(e.target.value, true)) editDraft(e.target.value); else setError("INPUT_LIMIT") }} onCompositionStart={() => { composing.current = true }} onCompositionEnd={() => { composing.current = false }} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !composing.current && !isComposing(event.nativeEvent)) { event.preventDefault(); send() } }} /><div className="composer-actions"><button className="quiet" onClick={() => void action("reset")} disabled={snapshot.applying}>{t.reset}</button><button className="quiet" onClick={() => void action(panel ? "compact" : "panel")} aria-label={panel ? t.compact : t.expand}>{panel ? "↙" : "↗"}</button><button className="quiet" onClick={() => void action("draft", draft).then(() => action("hide"))} aria-label={t.close}>×</button><span className="count">{Array.from(draft).length > 3200 ? Array.from(draft).length + "/4000" : ""}</span>{busy ? <button onClick={() => void action("stop")} disabled={snapshot.applying}>{t.stop}</button> : <button className="send" disabled={!snapshot.enabled || !draft.trim() || !snapshot.parent || snapshot.requiresNewConversation || Boolean(snapshot.readiness && snapshot.readiness.phase !== "ready")} onClick={send}>{t.send}</button>}</div></footer>
