...
export function UpdatesPage({ api, settings, run, busy }: SettingsPageProps & { settings: DesktopSettingsV1 }) {
  ...
  return <section><header className="page-header"><h1>{t("업데이트")}</h1><p>{t("Daemonlet을 최신 버전으로 유지하세요.")}</p></header>
    <section className="settings-card"><h2>Daemonlet for Codex {state?.currentVersion}</h2><p role="status" aria-live="polite">{state ? t(labels[state.phase]) : t("설정 불러오는 중…")}</p>
      {state?.version && <p>{t("새 버전")}: {state.version}</p>}
      {state?.reason && reasons[state.reason] && <p>{t(reasons[state.reason])}</p>}
      {state?.phase === "downloading" && <progress max={100} value={state.progress ?? 0} aria-label={t("업데이트 다운로드")} />}
      <div className="button-row">
        <button disabled={locked || state?.phase === "downloaded"} onClick={() => act({ action: "check" })}>{t("업데이트 확인")}</button>
        {state?.phase === "available" && state.candidateId && <button disabled={Boolean(busy)} className="primary" onClick={() => act({ action: "download", candidateId: state.candidateId! })}>{t("다운로드")}</button>}
        {state?.phase === "downloading" && <button onClick={() => void updates?.act({ action: "cancelDownload" })}>{t("다운로드 취소")}</button>}
        ...
        <button disabled={state?.phase === "preparing" || state?.phase === "handoff"} onClick={() => act({ action: "openRelease" })}>{t("배포 페이지 열기")}</button>
      </div>
    </section>
    <section className="settings-card"><label><input type="checkbox" checked={settings.updateAutoCheck} disabled={locked} onChange={event => void run("업데이트 설정 저장", () => api.updateSettings({ updateAutoCheck: event.target.checked }))} /> {t("하루에 한 번 새 버전 확인")}</label><p>{t("자동으로 다운로드하거나 재시작하지 않습니다.")}</p></section>
  </section>
}
