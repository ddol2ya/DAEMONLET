...
const voiceErrors = { UNAVAILABLE: "이 기기에서는 받아쓰기를 사용할 수 없어요.", PERMISSION_DENIED: "시스템 설정에서 마이크·음성 인식 접근을 허용해 주세요.", ON_DEVICE_UNAVAILABLE: "선택한 언어의 기기 내 음성 인식을 사용할 수 없어요.", AUDIO_UNAVAILABLE: "사용 가능한 마이크를 확인해 주세요.", RECOGNITION_FAILED: "음성을 인식하지 못했어요. 다시 시도해 주세요." }
...
export default function TaskControlPanel({ hidden, collapsed }: { hidden: boolean; collapsed: boolean }) {
  ...
  const feedback = t(notice) || (voiceActive ? t("Apple 음성 인식으로 받아쓰는 중 · 전송 전 수정할 수 있어요.") : snapshot?.needsClient ? t("승인·질문은 원래 Codex 대화에서 확인해 주세요.") : awaitingDesktop ? t("Codex 데스크톱이 열리면 자동으로 연결해요.") : snapshot?.issue ? t(errors[snapshot.issue]) : desktop && selected && !selected.canSend && selected.canStop ? t("실행 중 추가 지시는 원래 대화에서 보내 주세요. 종료 후 후속 질문을 보낼 수 있어요.") : selected ? t(labels[selected.state]) : snapshot?.connection === "ready" ? desktop ? t("데스크톱의 대화를 선택해 주세요.") : t("공유 서버의 대화를 선택해 주세요.") : t("데스크톱 자동 연결을 켜거나 공유 서버에 연결해 주세요."))
  return <main hidden={hidden} className={`task-bubble control-panel ${collapsed ? "compact" : ""}`} aria-label={t("Codex 작업 제어")} onPointerDownCapture={event => captureTarget(event.target)} onPointerUpCapture={releaseTarget} onPointerCancel={releaseTarget} onKeyDownCapture={event => { if (["Enter", " "].includes(event.key)) captureTarget(event.target) }} onKeyUpCapture={releaseTarget} onBlurCapture={event => { if (!event.currentTarget.contains(event.relatedTarget)) releaseTarget() }}>
    <div className="bubble-top">
      <span className={`task-dot ${snapshot?.connection !== "ready" ? "stale" : ""}`} aria-hidden="true" />
      <div className="task-heading"><strong title={collapsed && selected ? selected.title : undefined}>{collapsed && selected ? selected.title : t("Codex 제어")}</strong>{!collapsed && <span>{desktop ? snapshot?.connection === "ready" ? t("데스크톱 자동 연결됨") : snapshot?.autoConnect ? t("데스크톱 연결 대기") : t("데스크톱 연결 일시 중지") : snapshot?.connection === "ready" ? t("공유 서버 연결됨") : t("Codex 연결")}</span>}</div>
      {!collapsed && <button className="icon-button" aria-label={t("선택한 대화 열기")} title={t("선택한 대화를 ChatGPT에서 열기")} disabled={!selected?.canOpenConversation || pending || snapshot?.connection !== "ready"} onClick={() => api && selected && void perform(() => api.openConversation(selected.key), () => setNotice("ChatGPT에 해당 대화 열기를 요청했어요."))}>↗</button>}
      {!collapsed && <button className="icon-button" title={t("작업 알림으로 돌아가기")} aria-label={t("작업 알림으로 돌아가기")} disabled={busy} onClick={() => api && void perform(() => api.setView("activity", false))}>←</button>}
      <button className="icon-button" title={collapsed ? t("Codex 제어 펼치기") : t("Codex 제어 접기")} aria-label={collapsed ? t("Codex 제어 펼치기") : t("Codex 제어 접기")} aria-expanded={!collapsed} disabled={busy} onClick={() => api && void perform(() => api.setView("control", !collapsed))}>{collapsed ? "⌄" : "⌃"}</button>
    </div>
    {!collapsed && <>
      {snapshot?.connection !== "ready" ? <div className="control-connect">
        <div className="control-auto-row"><span>{t("Codex 데스크톱")}</span><button className="control-small" aria-label={snapshot?.autoConnect ? t("데스크톱 자동 연결 중지") : t("데스크톱 자동 연결")} disabled={busy || !api} onClick={() => api && void perform(() => snapshot?.autoConnect ? api.disconnect() : api.connectDesktop(), receive)}>{snapshot?.autoConnect ? t("연결 중지") : t("자동 연결")}</button></div>
        <details className="control-manual" open={manual} onToggle={event => setManual(event.currentTarget.open)}><summary>{t("공유 서버 직접 연결")}</summary>
          <label className="control-label sr-label" htmlFor="control-socket">{t("공유 서버 소켓")}</label>
          <div className="control-connect-row"><input id="control-socket" value={path} onChange={e => setPath(e.target.value)} placeholder="/tmp/codex-control/server.sock" spellCheck={false} disabled={busy} /><button className="control-small" disabled={busy || !path || !api} onClick={() => api && void perform(() => api.connect(path), receive)}>{busy ? t("연결 중") : t("연결")}</button></div>
        </details>
      </div> : <div className="control-select-row">
        <label className="control-label sr-label" htmlFor="control-thread">{t("제어할 대화")}</label>
        <select id="control-thread" aria-label={t("제어할 대화")} value={snapshot.selectedKey ?? ""} disabled={pending || voiceActive} onChange={e => { const key = e.target.value; if (key && api) void perform(() => api.select(key), value => { receive(value); if (!selected && drafts.unbound) setDrafts(old => ({ ...old, [key]: old[key] || old.unbound, unbound: "" })) }) }}>
          <option value="" disabled>{snapshot.threads.length ? t("제어할 대화 선택") : t("제어할 수 있는 대화가 없어요")}</option>
          {snapshot.threads.map(t => <option key={t.key} value={t.key}>{t.title}{t.project ? ` · ${t.project}` : ""}</option>)}
        </select>
        <button className="icon-button" aria-label={t("작업 새로고침")} title={t("새로고침")} disabled={pending} onClick={() => api && void perform(() => api.refresh(), receive)}>↻</button>
        <button className="icon-button" aria-label={t("Codex 연결 해제")} title={t("연결 해제 · 자동 연결 일시 중지")} disabled={pending} onClick={() => api && void perform(() => api.disconnect(), receive)}>×</button>
      </div>}
      ...
      <textarea ref={textarea} aria-label={t("후속 질문")} placeholder={t("후속 질문을 입력하거나 마이크를 눌러 말해 보세요")} value={draft} maxLength={4000} disabled={voiceActive || pending} onCompositionStart={() => setComposing(true)} onCompositionEnd={() => setComposing(false)} onChange={e => { setDrafts(old => ({ ...old, [draftKey]: e.target.value })); sendAttempt.current = null }} onKeyDown={e => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey) && !e.nativeEvent.isComposing) { e.preventDefault(); send() } }} />
      <div className="control-actions">
        <button className={`voice-button ${voiceActive ? "recording" : ""}`} aria-label={voiceActive ? t("받아쓰기 중지") : t("음성 받아쓰기")} title={voiceActive ? t("받아쓰기 중지") : t("음성 받아쓰기")} aria-pressed={voiceActive} disabled={busy || !api || voice?.state === "stopping"} onClick={toggleVoice}><Mic stop={voiceActive} /><span>{voice?.state === "starting" ? t("준비 중") : voice?.state === "stopping" ? t("정리 중") : voiceActive ? t("듣는 중") : t("받아쓰기")}</span></button>
        <div className="control-send-actions"><button className="control-stop" aria-label={t("선택한 작업 실행 중지")} title={t("선택한 작업 실행 중지")} disabled={!selected?.canStop || pending} data-control-action="stop" onClick={stop}><Mic stop /></button><button className="task-action" data-control-action="send" disabled={composing || !selected?.canSend || !draft.trim() || voiceActive || pending} onClick={send}>{t("전송 ")}<span aria-hidden="true">↑</span></button></div>
      </div>
    </>}
  </main>
}
